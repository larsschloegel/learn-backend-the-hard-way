...
Assignment
One of the calls to get_player_record will raise a player id not found exception. Wrap all five calls in a single try-except block. If an exception is raised, print the exception. Note: execution of the try block stops at the first exception, so later calls won't run!